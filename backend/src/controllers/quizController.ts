...
export async function updateQuizSettings(
  ...
  try {
    ...
    if (quiz.status === "ACTIVE" || quiz.status === "ENDED") {
      throw new AppError(
        "Cannot change settings once the competition has started.",
        409
      );
    }
...
/**
 * Checks the readiness rules from spec section 25:
 * - Exact required total number of questions
 * - Exact Medium question count
 * - Exact Hard question count
 * - Every question must have all options filled
 *
 * Used before publishing and activating a quiz.
 */
async function checkReadiness(quizId: string) {
  ...
  if (total !== quiz.requiredTotal) {
    problems.push(
      `Quiz must contain exactly ${quiz.requiredTotal} questions (currently ${total}).`
    );
  }

  if (
    mediumCount !== quiz.requiredMedium ||
    hardCount !== quiz.requiredHard
  ) {
    problems.push(
  ...
  for (const q of questions) {
    ...
    if (opts.some((o) => !o.trim())) {
      problems.push(
        `Question "${q.questionText.slice(0, 40)}..." is missing an option.`
      );
    }
  }
...
export async function getQuizReadiness(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function publishQuiz(
  ...
  try {
    ...
    if (!readiness.ready) {
      throw new AppError(
        `Quiz is not ready to publish: ${readiness.problems.join(" ")}`,
        422
      );
    }
...
export async function startCompetition(
  ...
  try {
    ...
    if (quiz.status !== "PUBLISHED") {
      throw new AppError(
        "Only a PUBLISHED quiz can be started.",
        409
      );
    }

    // Only one quiz should be ACTIVE at a time.
    // The participant flow identifies the active quiz by status.
    const alreadyActive = await prisma.quiz.findFirst({
      where: {
        status: "ACTIVE",
      },
    });
    ...
    if (alreadyActive && alreadyActive.id !== id) {
      throw new AppError(
        `Quiz "${alreadyActive.name}" is already active. End it first.`,
        409
      );
    }
...
export async function endCompetition(
  ...
  try {
    ...
    if (quiz.status !== "ACTIVE") {
      throw new AppError(
        "Only an ACTIVE quiz can be ended.",
        409
      );
    }
